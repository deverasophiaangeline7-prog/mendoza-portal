   ...
   public function storeGroup(Request $request)
    {
        ...
        $groupUser->password    = \Illuminate\Support\Facades\Hash::make(\Illuminate\Support\Str::random(16));
        ...
        \Illuminate\Support\Facades\DB::table('group_members')->insert($membersData);

        // 3. Send an initial system message
        Message::create([
            'sender_id'   => \Illuminate\Support\Facades\Auth::id(),
            'receiver_id' => $groupUser->user_id,
            'content'     => \Illuminate\Support\Facades\Auth::user()->name . ' created the group: ' . $request->group_name,
            'is_read'     => false,
        ]);
       ...
       // STRICT CHECK: Sender is Parent AND Receiver is NOT a Group Chat
        if ($apiKey && strtolower(Auth::user()->role) === 'parent' && $receiver && is_null($receiver->custom_name)) {
           ...
           *** MENDOZA ACADEMY CHEAT SHEET ***\n\n"
               ...
               . $eventsKnowledge;
           // UPDATED MODEL: Using the actual valid 2.5-flash model
          // Fixed code:
$url = 'https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent?key=' . $apiKey;
           ...
           if (strpos($aiText, 'IGNORE') !== false) {
               // Do absolutely nothing. The AI stays silent.
           } else {
               Message::create([
   ...
   private function getChatHistoryUsers($authId)
    {
        // 1. Fetch Normal 1-on-1 Chats (Filters out groups by checking if custom_name is NULL)
        $individualChats = User::where('user_id', '!=', $authId)
        ...
        // 2. Fetch Group Chats where the user is a member
        $groupUserIds = \Illuminate\Support\Facades\DB::table('group_members')
